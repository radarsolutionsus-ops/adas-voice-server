...
function updateExistingRow(sheet, rowNum, data) {
  ...
  const shopName = data.shop_name || data.shopName || data.shop || curr[COL.SHOP_NAME] || '';
  const vin = data.vin || curr[COL.VIN] || '';
...
'    .doc-status { display: flex; align-items: center; gap: 8px; padding: 10px 14px; border-radius: 8px; margin-bottom: 8px; font-size: 13px; }' +
...
function openApprovalSidebar() {
  ...
  const hasRevvPdf = revvPdfUrl && revvPdfUrl.startsWith('http');
